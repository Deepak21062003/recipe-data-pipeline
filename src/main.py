...
def process_recipe(recipe: dict) -> dict:
    ...
    raw_instr = ensure_list(recipe.get("instructions") or [])
    
    # Filter out empty or whitespace-only steps, and noise symbols
    def is_meaningful_step(s):
        if not s or not isinstance(s, str): return False
        s = s.strip()
        if not s: return False
        # If it's just noise symbols, it's not meaningful
        if re.match(r'^[▢\s\-*]+$', s): return False
        return True

    all_raw_steps = [s for s in (raw_prep + raw_cook + raw_instr) if is_meaningful_step(s)]
    ...
    if all_raw_steps:
        ...
        # Fallback if AI fails or returns noise
        if not final_prep and not final_cook:
            # If AI classification failed to find any prep/cook steps, 
            # treat them as prep steps as a safe bet for deterministic cleaning
            final_prep = clean_instructions(all_raw_steps)

    # If still empty (or no original steps), call Content Generation
    if not final_prep and not final_cook:
        ...
        metadata["ai_generated_instructions"] = False

    # Summarize & Structuring (Always provide a combined string)
    combined_instructions = ai_processor.summarize_steps(final_prep, final_cook)

    # FINAL EMERGENCY GUARDRAIL: Instructions MUST NOT be empty
    if not combined_instructions or not combined_instructions.strip():
        logger.warning(f"Instructions still empty for '{title}' after all processing. Using emergency fallback.")
        combined_instructions = "1. Prepare ingredients.\n2. Cook according to standard preference for " + title + "."
        metadata["emergency_fallback"] = True

    if is_drafted:
        disclaimer = "NOTE: These instructions were automatically generated by an AI model because the original recipe was missing them.\n\n"
        combined_instructions = disclaimer + combined_instructions

    metadata["prep_steps"] = final_prep
    metadata["cook_steps"] = final_cook
    ...
    return {
        ...
        "prep_steps": final_prep,
        "cook_steps": final_cook,
        "metadata": metadata,
        "servings": servings,
        **times
    }
